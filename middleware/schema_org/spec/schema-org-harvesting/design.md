...
3. **Inject the dataset implementation class into `XmlSitemap` instead of hardcoding `DummyDataset`**
...
4. **Implement `XmlSitemap` as the XML sitemap parser for the `xml` sitemap type**
...
5. **Keep `run_plugin()` as an async generator that yields `str | HarvesterError`**
...
6. **Use a string-based forward annotation for `PluginConfig` in `schema_org.plugin.py`**
