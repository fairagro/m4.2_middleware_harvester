...
- [ ] Align resulting identifiers and gracefully handle alignment faults yielding out a `RecordProcessingError` when a chunk parse fails.
...
- Broken XML responses or invalid attributes natively trigger `SemanticError` or `ValueError`, bundled iteratively as `RecordProcessingError` so the harvester doesn't exit entirely.
