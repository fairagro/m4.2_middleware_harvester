...
- [ ] Raise a `LinkedDataDatasetError` when the HTTP request fails.
- [ ] Raise a `LinkedDataDatasetError` when the HTML contains no `<script type="application/ld+json">` blocks.
...
- A JSON-LD block that is valid JSON but not valid JSON-LD → rdflib raises; propagate as `LinkedDataDatasetError`.
- HTTP error response → raise `LinkedDataDatasetError` with the URL and error detail.
...
- All retries exhausted → `NiceHttpClient.retry_get` raises; `_fetch_html` wraps it in `LinkedDataDatasetError`; the dataset is skipped and harvesting continues.
