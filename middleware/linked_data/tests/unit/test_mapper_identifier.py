...
def test_openagrar_propertyvalue_doi_is_investigation_identifier_when_no_source_url() -> None:
    ...
    harvested = GeneralSchemaOrgMapper().map_graph(graph, NO_DISCOVERY)
    identifier = root_identifier(harvested.arc_json)
...
def test_openagrar_propertyvalue_doi_is_stable_across_parses() -> None:
    first = root_identifier(
        GeneralSchemaOrgMapper().map_graph(parse_jsonld(OPENAGRAR_PROPERTYVALUE_DOI), NO_DISCOVERY).arc_json
    )
...
def test_openagrar_without_doi_uses_sanitized_source_url_without_pattern() -> None:
    ...
    source_url = "https://www.openagrar.de/receive/openagrar_mods_00107322"
    harvested = GeneralSchemaOrgMapper().map_graph(graph, MappingContext(source_url=source_url))
    identifier = root_identifier(harvested.arc_json)
...
def test_http_dataset_id_is_kept_as_identifier() -> None:
    ...
    identifier = root_identifier(GeneralSchemaOrgMapper().map_graph(graph, NO_DISCOVERY).arc_json)
...
def test_multiple_schema_urls_pick_lexicographic_minimum_regardless_of_graph_order() -> None:
    ...
    first = root_identifier(
        ...
        .map_graph(build(["https://example.org/zeta", "https://example.org/alpha"]), NO_DISCOVERY)
        .arc_json
    )
    second = root_identifier(
        GeneralSchemaOrgMapper()
        .map_graph(build(["https://example.org/alpha", "https://example.org/zeta"]), NO_DISCOVERY)
        .arc_json
    )
...
def test_multiple_schema_urls_with_casefold_ties_are_stable() -> None:
    ...
    first = root_identifier(GeneralSchemaOrgMapper().map_graph(build(urls_a), NO_DISCOVERY).arc_json)
    second = root_identifier(GeneralSchemaOrgMapper().map_graph(build(urls_b), NO_DISCOVERY).arc_json)
...
def test_multi_doi_harvest_identifier_stable_under_permuted_jsonld_order() -> None:
    ...
    first = root_identifier(mapper.map_graph(parse_jsonld(first_payload), ctx).arc_json)
    second = root_identifier(mapper.map_graph(parse_jsonld(second_payload), ctx).arc_json)
...
def test_single_doi_without_source_url_has_no_alternate_identifier_comment() -> None:
    harvested = GeneralSchemaOrgMapper().map_graph(parse_jsonld(OPENAGRAR_PROPERTYVALUE_DOI), NO_DISCOVERY)
...
def test_shared_doi_on_two_pages_uses_distinct_harvest_identifiers() -> None:
    ...
    id_a = root_identifier(
        mapper.map_graph(graph, MappingContext(source_url=url_a, harvest_source_id="openagrar_mods_00088718")).arc_json
    )
    id_b = root_identifier(
        mapper.map_graph(graph, MappingContext(source_url=url_b, harvest_source_id="openagrar_mods_00109919")).arc_json
    )
    ...
    assert (
        ...
        in mapper.map_graph(
            graph, MappingContext(source_url=url_a, harvest_source_id="openagrar_mods_00088718")
        ).arc_json
    )
...
def test_shared_doi_with_generic_source_url_uses_sanitized_page_url() -> None:
    ...
    identifier = root_identifier(
        mapper.map_graph(graph, MappingContext(source_url="https://example.org/generic-page")).arc_json
    )
...
def test_sorcering_pair_pages_keep_distinct_harvest_identifiers() -> None:
    ...
    id_a = root_identifier(
        mapper.map_graph(
            graph_a, MappingContext(source_url=url_a, harvest_source_id="openagrar_mods_00100605")
        ).arc_json
    )
    id_b = root_identifier(
        mapper.map_graph(
            graph_b, MappingContext(source_url=url_b, harvest_source_id="openagrar_mods_00108456")
        ).arc_json
    )
