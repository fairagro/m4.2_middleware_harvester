...
def test_html_jsonld_dataset_raises_on_no_jsonld_blocks() -> None:
    ...
    with pytest.raises(LinkedDataDatasetError, match="No JSON-LD blocks"):
        asyncio.run(run())
...
def test_html_jsonld_dataset_raises_on_http_error() -> None:
    ...
    with pytest.raises(LinkedDataDatasetError, match="HTTP 404"):
        asyncio.run(run())
...
def test_html_jsonld_dataset_raises_on_invalid_json() -> None:
    ...
    with pytest.raises(LinkedDataDatasetError, match="Invalid JSON"):
        asyncio.run(run())
...
<body></body>
    ...
    with pytest.raises(LinkedDataDatasetError) as exc_info:
        asyncio.run(run())
