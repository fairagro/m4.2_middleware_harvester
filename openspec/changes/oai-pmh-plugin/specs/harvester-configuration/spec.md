...
- **WHEN** a repository entry sets only `oai_pmh` (plus shared fields and `mapper` as required)
...
### Requirement: oai_pmh repositories require mapper config

The system SHALL require a top-level repository `mapper` block when the `oai_pmh` plugin key is selected, and SHALL fail
...
- **WHEN** a repository entry sets exactly one plugin key `oai_pmh` and a `mapper` with a supported `type`
- **THEN** configuration validation succeeds
...
the configured shared parser’s `produces` kind, typically `rdf_graph`). Unsupported mapper types MUST fail fast at
startup.
