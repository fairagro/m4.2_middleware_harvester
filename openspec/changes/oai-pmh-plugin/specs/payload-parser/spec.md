...
and/or per record — not by inheritance between parser and mapper classes.
...
- **WHEN** an `oai_pmh` repository configures a parser and mapper with mismatched kinds
...
DataMapper mapping logic. Parsers that require HTTP SHALL raise a descriptive error when no client is provided. Parsers
that consume inline discovery payloads MUST NOT require an HTTP client.
