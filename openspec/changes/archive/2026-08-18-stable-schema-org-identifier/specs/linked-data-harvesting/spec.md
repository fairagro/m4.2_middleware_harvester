...
URL for HTML JSON-LD / MyCoRe Receive-URL) as mapping context so the mapper can use it as a stable identifier fallback
when the graph has no DOI and no `http(s)` `url`/`sameAs`/`@id`.
