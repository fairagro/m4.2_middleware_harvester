...
**Not adopted.** Issue `#227` and PR `#248` were closed as not-planned on 2026-09-21.
@Zalfsten chose not to introduce the per-RDI overlay mechanism described below.

This folder is archived as a design record only. **Its spec deltas under `specs/` were
deliberately NOT merged into `openspec/specs/`** — the requirements they describe are not
in force, because the approach they belong to was rejected.

What did ship from this design:

- The two behaviour-neutral D4 dialect seams (`LinkedDataMapper._stable_wrap` as an
  instance method, and the dialect hook) were salvaged into PR `#305` and are on `main`.
- OpenAgrar's title-fallback cascade stayed in the shared `GeneralSchemaOrgMapper` and
  landed via `#164` / PR `#221`, not via an overlay.
