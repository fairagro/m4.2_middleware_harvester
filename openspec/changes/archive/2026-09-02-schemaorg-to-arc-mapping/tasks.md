...
- [x] 1.1 Add `SCHEMAORG_CONTEXT_ALLOWLIST` frozen set to `linked_data_mapper/general_schema_org_mapper.py` (or a
      sibling module)
...
- [x] 1.4 Add unit tests: valid HTTPS context, valid HTTP context (normalized), mixed http/https, Bioschemas extension,
      unknown context error

## 2. Multi-Dataset Handling

- [x] 2.1 Extend `_find_dataset_subject` → `_find_dataset_subjects` returning `list[Node]` (all `schema:Dataset`
      subjects in either http/https namespace)
...
- [x] 3.1 Extend Investigation + `_create_assay_table` to iterate `schema:distribution` → `schema:DataDownload`
      resources
- [x] 3.2 Add Investigation `"Distribution"` comments (`encodingFormat: contentUrl`, skip empty `contentUrl`)
- [x] 3.3 Add one Measurement `"Distribution"` comment column with joined labels (ARCtrl: no multi-output-column
      Measurement row)
- [x] 3.4 Add unit test: single DataDownload → Investigation + Measurement `"Distribution"` comments
- [x] 3.5 Add unit test: multiple DataDownload → joined Measurement cell (no extra output columns)
