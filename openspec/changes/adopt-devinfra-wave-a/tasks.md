...
- [x] 1.6 Diff local `.agents/skills/arctrl/` vs Devinfra; then copy verbatim
...
- [x] 3.1 Create `openspec/principles.md` as P3 product overlay (extends
      ...
      examples section)
- [x] 3.2 Delete `openspec/specs/principles/`; update all references
      (`AGENTS.md`, `openspec/config.yaml`, and any other cites)
- [x] 3.3 Create local `docs/surface-quality-bar.md` with the two product rows
...
- [x] 3.4 Update `AGENTS.md` pointers (`.global` + local principles; surface-bar
      global + local; vendor set; shared `arctrl`; keep `config-wrapper`;
      `m42-ai` / `scripts/ai`)
- [x] 3.5 Ruff cutover: sync Devinfra `ruff.toml` → `ruff.global.toml`; add
      product `ruff.toml` (`extend` + test `D103` + scripts/ai ignores + preview
      rule ignores not yet green); remove `[tool.ruff*]` from `pyproject.toml`;
      point `.vscode` / pre-commit comments at `ruff.toml`; add
      `.markdownlintignore` / `.prettierignore` for vendor skills; `ruff format`
      any middleware files the new formatter requires
...
- [x] 4.5 Run `openspec validate --change adopt-devinfra-wave-a` (and repo
