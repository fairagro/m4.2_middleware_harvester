...
dataset, or mapper formats automatically. The plugin MUST NOT accept a `payload_type` field.
...
#### Scenario: payload_type is rejected

- **WHEN** linked-data plugin config still sets `payload_type`
- **THEN** validation fails at startup (unknown / forbidden field) or the field is absent from the model so YAML load
  fails closed
