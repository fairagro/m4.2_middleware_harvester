...
- [x] 2.1 Grep for tests that clearly need `system_local` or `system_external` today
- [x] 2.2 Apply remaps only if needed; otherwise document zero remaps in the PR

## 3. Verify + ready signal

- [x] 3.1 Smoke: `uv run pytest --markers` lists both new markers; collect with
