...
The system SHALL ensure that support `SitemapType.xml` in plugin configuration.
...
The system SHALL ensure that accept a single sitemap entry point URL in plugin configuration.
...
The system SHALL ensure that support both `urlset` and `sitemapindex` document roots.
...
The system SHALL ensure that recursively follow nested sitemap indexes.
...
The system SHALL ensure that prevent sitemap loops by tracking already visited sitemap URLs.
...
The system SHALL ensure that deduplicate discovered dataset URLs before yielding results (as `SkippedRecord`).
...
The system SHALL ensure that yield `RecordProcessingError` for empty `<loc>` elements (do not silently skip).
...
The system SHALL ensure that yield one `UrlDiscoveryResult` per unique dataset URL found in a `urlset`.
...
The system SHALL ensure that use safe XML parsing (`defusedxml`) for untrusted content.
...
The system SHALL ensure that fail fast with `LinkedDataSitemapError` when XML parsing fails (malformed.
