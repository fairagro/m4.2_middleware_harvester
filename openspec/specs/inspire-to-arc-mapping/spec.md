...
The system SHALL ensure that map each `InspireRecord` to exactly one `ArcInvestigation` with title, description, contacts, publications, and ontology annotations as defined in the authoritative mapping source.
...
The system SHALL ensure that create one `ArcStudy` per record containing a Spatial Sampling protocol (omitted for `nonGeographicDataset`) and a Data Acquisition protocol.
...
The system SHALL ensure that create one `ArcAssay` per record containing a Data Processing protocol.
...
The system SHALL ensure that skip the Spatial Sampling protocol when `record.hierarchy == "nonGeographicDataset"`.
...
The system SHALL ensure that skip records whose hierarchy is not in `["dataset", "series", "nonGeographicDataset"]`.
