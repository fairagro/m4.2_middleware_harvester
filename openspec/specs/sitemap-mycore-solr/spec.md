...
The system SHALL construct the dataset HTML page URL as `{scheme}://{host}/receive/{id}` where scheme and host are derived from `sitemap_url`.
...
The system SHALL support Solr pagination: when `numFound > start + len(docs)`, issue further requests by incrementing the `start` parameter until all pages are consumed.
...
The system SHALL raise `LinkedDataSitemapError` when the JSON body is not an object or lacks the expected `response` envelope (`numFound`, `start`, `docs`).
