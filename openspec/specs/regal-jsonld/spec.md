...
The system SHALL ensure that support a dedicated Regal sitemap type in plugin configuration.
...
The system SHALL ensure that accept a Regal `/find` endpoint in `sitemap_url`; query parameters are optional.
...
The system SHALL ensure that always set `format=json` and pagination `from` in software; ignore those keys on `sitemap_url`.
...
The system SHALL ensure that use optional `resource_base_url` to expand compact Regal `@id` values to absolute IRIs; when unset, derive `{scheme}://{host}/resource/` from `sitemap_url`.
...
The system SHALL ensure that issue HTTP GET requests to the constructed `/find` URL.
...
The system SHALL ensure that yield one discovery result per record that carries the record's JSON-LD payload inline (no follow-up HTML fetch).
...
The system SHALL ensure that use each record's `@id` as the stable discovery identity for deduplication and error reporting.
...
The system SHALL ensure that deduplicate discovered records by `@id` within a single harvest run.
...
The system SHALL ensure that paginate by advancing the `from` query parameter by the number of records returned until a page is empty or shorter than the requested page size.
...
The system SHALL ensure that expose `get_expected_count()` as unknown (`None`) when the `/find` response does not provide a total hit count.
...
The system SHALL ensure that raise `LinkedDataSitemapError` when the response body is not a JSON array (fatal discovery failure; not a per-record yield).
...
The system SHALL ensure that do not silently drop malformed or unusable `/find` array entries; yield `RecordProcessingError` (shared harvester type, same as inspire CSW) so the orchestrator can update the harvest report.
...
The system SHALL ensure that treat a record object missing `@id` as a record-level failure (not a deliberate skip): yield `RecordProcessingError` so `failed_datasets` increments and `fairagro:failedRecords` gains an entry.

#### Scenario: Satisfies — Treat a record object missing @id as a record-level failure…
- **WHEN** the conditions described by this requirement apply
- **THEN** Treat a record object missing `@id` as a record-level failure (not a deliberate skip): yield `RecordProcessingError` so `failed_datasets` increments and `fairagro:failedRecords` gains an entry

### Requirement: Treat a non-object JSON array element as a record-level failure:…
The system SHALL ensure that treat a non-object JSON array element as a record-level failure: yield `RecordProcessingError` the same way (message must identify page offset / array index when no `@id` exists).
...
The system SHALL ensure that support a dedicated Regal dataset type in plugin configuration.
...
The system SHALL ensure that construct the dataset from an inline Regal JSON-LD discovery result without requiring an HTTP client.
...
The system SHALL ensure that use the record `@id` as the stable dataset identifier (DOI is a publication attribute, not the discovery identity).
...
The system SHALL ensure that raise a dataset error when the discovery result type is unsupported.
...
The system SHALL ensure that raise a dataset error when the payload is not valid JSON-LD for rdflib.
...
The system SHALL ensure that support a dedicated Regal payload type that selects a Regal→ARC mapper.
...
The system SHALL ensure that map Regal `ResearchData` graphs to serialized ARC RO-Crate JSON-LD as defined in [`docs/regal_mapping.md`](../../../../docs/regal_mapping.md) and [`regal-to-arc-mapping`](../regal-to-arc-mapping/spec.md).
...
The system SHALL ensure that keep Regal mapping logic separate from schema.org mapping implementations (`GeneralSchemaOrgMapper`).
...
The system SHALL ensure that yield a mapping error (as `HarvesterError`) when the graph lacks mappable ResearchData metadata; do not crash the plugin.
...
The system SHALL ensure that yield `SkippedRecord` for duplicate discovery identifiers (counts toward `skipped_datasets`, not failures).
...
The system SHALL ensure that yield `RecordProcessingError` for unusable discovery entries (same shared type as inspire); the plugin forwards them to the orchestrator without a plugin-local failure wrapper type.
