...
Configures TLS certificate verification for INSPIRE CSW connections so operators can harvest from endpoints with
...
The system SHALL expose `inspire.Config.verify_ssl` typed as `bool | str` with default `True`. A `bool` SHALL enable
(`True`) or disable (`False`) TLS certificate verification. A `str` SHALL be interpreted as a filesystem path to a CA
bundle used for verification.
...
- **WHEN** configuration sets `verify_ssl` to a boolean-like string such as `"false"` or `"true"` (quoted YAML or
...
The system SHALL ensure that `CSWClient.connect()` (and its underlying connection setup) constructs
`CatalogueServiceWeb` with `auth=Authentication(verify=config.verify_ssl)` so the configured verification setting
applies to all subsequent OWSLib CSW HTTP requests for that client.
...
The system SHALL log a WARNING when connecting with `verify_ssl` set to `False`, stating that TLS certificate
verification is disabled for the CSW endpoint.
