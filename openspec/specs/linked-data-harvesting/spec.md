...
When mapping a Schema.org graph, the linked-data plugin MUST supply the dataset's discovery identifier (the fetched page URL) as mapping context so the mapper can use it as a stable identifier fallback when the graph has no DOI and no `http(s)` `url`/`sameAs`/`@id`.
