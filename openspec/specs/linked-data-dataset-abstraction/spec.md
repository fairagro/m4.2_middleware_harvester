...
The system SHALL ensure that keep dataset wrappers independent of sitemap discovery and HTTP fetching.
...
The system SHALL ensure that use the dataset identifier as the stable key for error reporting and downstream mapping.
...
The system SHALL ensure that require every successful `DiscoveryResult` to expose a stable `identifier` used for sitemap-level deduplication.
...
The system SHALL ensure that deduplicate successful discovery identifiers in `Sitemap.discover()` and yield shared `SkippedRecord` (not a plugin-local duplicate type).
...
The system SHALL ensure that yield shared `RecordProcessingError` for unusable discovery entries (missing id, non-object payload, empty loc, …)—same type as inspire; do not introduce a plugin-local failure wrapper.
...
The system SHALL ensure that have the plugin forward `RecordProcessingError` and `SkippedRecord` from discovery to the orchestrator unchanged.
